...
class UNet2D(nn.Module): # 3 output logits represent the probability of each pixel belonging to each tumor region (ET, TC, WT)
    def __init__(self, in_channels: int = 4, out_channels: int = 3, features: tuple[int, ...] = (8, 16, 32, 64, 128, 256)) -> None:
        ...
        self.up1 = UpBlock(features[5] * 2, features[5], features[5])
        self.up2 = UpBlock(features[5], features[4], features[4])
        self.up3 = UpBlock(features[4], features[3], features[3])
        self.up4 = UpBlock(features[3], features[2], features[2])
        self.up5 = UpBlock(features[2], features[1], features[1])
        self.up6 = UpBlock(features[1], features[0], features[0])
